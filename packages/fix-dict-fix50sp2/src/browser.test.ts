...
//
// Browser-environment smoke test: the full engine + full FIX 5.0 SP2 dictionary must parse,
...
describe('browser environment (happy-dom)', () => {
  ...
  it('parses, validates, and re-encodes a FIXT.1.1 Logon round-trip', () => {
    const fix = createFixEngine(dictionary);
    const SOH = '\x01';
    // A FIXT.1.1 Logon frame: tag 8 carries the TRANSPORT version; the application
    // version rides on DefaultApplVerID(1137) = 9 (FIX 5.0 SP2).
    const raw = [
      '8=FIXT.1.1',
      '9=0',
      `35=${MsgType.Logon}`,
    ...
    // Parsing never throws; issues are returned data — and the FIXT BeginString matches
    // the dictionary, so no begin-string-mismatch warning.
    expect(issues.filter((i) => i.code === 'parse/begin-string-mismatch')).toEqual([]);

    const problems = fix.validate(message);
    expect(Array.isArray(problems)).toBe(true);
    expect(problems.filter((i) => i.severity === 'error')).toEqual([]);
    ...
    const wire = fix.encode({
      ...
      fields: {
        ...
        [Tags.MsgSeqNum]: '1',
        [Tags.SendingTime]: '20260801-00:00:00.000',
        [Tags.EncryptMethod]: '0',
        [Tags.HeartBtInt]: '30',
        [Tags.DefaultApplVerID]: '9',
      },
    });
    expect(wire.startsWith(`8=FIXT.1.1${SOH}`)).toBe(true);
    ...
    expect(reparsed.issues.filter((i) => i.code === 'parse/checksum-mismatch')).toEqual([]);
  });
});
