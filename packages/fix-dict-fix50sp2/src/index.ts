...
import { inboundTypeGuard, messageFactory, messageTypeGuard } from '@boarteam/fix';
import type {
  DictionaryJSON,
  InboundMessageOf,
  MessageFactory,
...
// `isMessageType`/`MessageOf` narrow and annotate a message you BUILT, and
// `isInboundType`/`InboundOf` are their counterparts for one you RECEIVED.
...
/**
 * The narrowing guard for a RECEIVED message, bound to this dictionary's `MessageBodies` —
 * `isMessageType` for the inbound path. Given a message of unknown body (an
 * `InboundMessage`, which is what `toInbound`/`engine.inbound` hand back) and a `MsgType`
 * value, it narrows the view so `get()` reads are typed to that message's body — and unlike
 * the encode-side guard it preserves the inbound view, so `envelope` and `parsed` survive
 * the narrowing. Runtime is a plain `msgType` compare.
 *
 * ```ts
 * const inbound = toInbound(parse(raw, dictionary).message, dictionary);
 ...
 * ```
 *
 * To narrow to *every* message this dictionary defines at once — a
 * `switch (message.msgType)` that types each case — bind `inboundKnownGuard` yourself:
 * it takes the dictionary at runtime, so an app that extends this one must bind its own
 * extended dictionary rather than borrow a guard bound to the stock one.
 */
export const isInboundType = inboundTypeGuard<MessageBodies>();

/**
 * The typed read surface of a RECEIVED message whose `MsgType` value is `M` — the inbound
 * counterpart of `MessageOf`, for annotating a narrowed message (a handler parameter, a
 * variable). `InboundOf<'W'>` is the market-data snapshot's read view, `msgType` pinned to
 * the literal `'W'` and `envelope`/`parsed` included.
 */
export type InboundOf<M extends keyof MessageBodies & string> = InboundMessageOf<
  MessageBodies[M] & object,
  M
>;
