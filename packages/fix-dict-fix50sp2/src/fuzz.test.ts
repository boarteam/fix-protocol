 ...
 * real FIX 5.0 SP2 (FIXT-framed) dictionary, plus a deterministic byte fuzzer. Determinism:
 * a seeded LCG (no `Math.random`) so a failure reproduces exactly.
 */
...
/** A known-good FIXT Heartbeat frame to mutate (encode computes 8/9/10). */
function baseMessage(): string {
  const msg: EncodeMessage = {
    msgType: '0',
    fields: { 49: 'SENDER', 56: 'TARGET', 34: '1', 52: '20260801-12:30:00.000', 112: 'TEST-1' },
  };
...
describe('adversarial — round-trip identity (sanity anchor)', () => {
  it('the base FIXT message round-trips byte-stably and validates clean', () => {
    const base = baseMessage();
    expect(base.startsWith('8=FIXT.1.1|')).toBe(true);
...
describe('adversarial — targeted mutations', () => {
  ...
  it('survives junk tags and segments', () => {
    ...
    for (const j of junk) {
      expectSaneParse(`8=FIXT.1.1|9=5|35=0|${j}|10=000|`);
  ...
  it('survives oversized group counters without allocating', () => {
    ...
    const r = expectSaneParse(`8=FIXT.1.1|9=5|35=W|262=R|268=100000000|10=000|`);
  ...
  it('handles a genuinely large number of real group entries linearly', () => {
    ...
    const wire = `8=FIXT.1.1|9=5|35=W|262=R|268=${n}|${entries.join('|')}|10=000|`;
    const started = performance.now();
  ...
  it('survives deeply repeated separators and equals signs', () => {
    ...
    expectSaneParse('='.repeat(5000));
    expectSaneParse(('8=FIXT.1.1|' + '='.repeat(1000) + '|').repeat(10));
  });
});
  ...
  it('never throws on random SOH-delimited field streams', () => {
    ...
    for (let trial = 0; trial < 1500; trial++) {
      ...
      for (let i = 0; i < nFields; i++) {
        const tag = rng() < 0.7 ? `${randInt(rng, 1600)}` : randToken();
...
describe('adversarial — parseAll on concatenated and broken FIXT streams', () => {
