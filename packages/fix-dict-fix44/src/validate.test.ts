...
// repeating groups (W has NoMDEntries; AE has groups) and nested structure.
const MESSAGE_TYPES = ['0', '1', '2', '3', '4', '5', 'A', 'D', 'V', 'W', 'X', 'AE', 'j'];

describe('validate over the full FIX 4.4 dictionary', () => {
  ...
  it('flags a required field dropped from a required group entry (deep, indexed path)', () => {
    ...
    // carry MDEntryType(269). Drop it from the built entry and assert the indexed path. This is
    // hand-authored (not via buildContainer) so the validator's own traversal is checked.
