 ...
 * real FIX 4.2 dictionary, plus a deterministic byte fuzzer. Determinism: a seeded LCG (no
...
describe('adversarial — targeted mutations', () => {
  ...
  it('survives junk tags and segments', () => {
    ...
    for (const j of junk) {
      expectSaneParse(`8=FIX.4.2|9=5|35=0|${j}|10=000|`);
  ...
  it('survives oversized group counters without allocating', () => {
    ...
    const r = expectSaneParse(`8=FIX.4.2|9=5|35=W|262=R|268=100000000|10=000|`);
  ...
  it('handles a genuinely large number of real group entries linearly', () => {
    ...
    const wire = `8=FIX.4.2|9=5|35=W|262=R|268=${n}|${entries.join('|')}|10=000|`;
    const started = performance.now();
  ...
  it('survives deeply repeated separators and equals signs', () => {
    ...
    expectSaneParse('='.repeat(5000));
    expectSaneParse(('8=FIX.4.2|' + '='.repeat(1000) + '|').repeat(10));
  });
});
